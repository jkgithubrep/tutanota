 ...
 */
export async function getCachedLibPath({ rootDir, nodeModule, environment, platform }, log) {
	...
	return buildCachedLibPath({ rootDir, nodeModule, environment, versionedEnvironment, platform, libraryVersion })
}

export function buildCachedLibPath({ rootDir, nodeModule, environment, versionedEnvironment, platform, libraryVersion }) {
	const dir = path.join(rootDir, "native-cache", environment)
	return path.resolve(dir, `${nodeModule}-${libraryVersion}-${versionedEnvironment}-${platform}.node`)
}
