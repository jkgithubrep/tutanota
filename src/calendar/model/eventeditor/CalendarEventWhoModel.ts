 ...
 * tracks external passwords, attendance status, list of attendees, recipients to invite, update, cancel.
 */
export class CalendarEventWhoModel {
	...
	private setupAttendees(initialValues: Partial<Stripped<CalendarEvent>>) {
		...
		for (const attendee of initialValues.attendees?.slice().map((a) => ({
			...a,
		...
		if (initialOrganizer != null) {
			const organizerAttendee = this.initialAttendees.get(initialOrganizer.address)
			this._organizer =
				organizerAttendee ??
				createCalendarEventAttendee({
					address: initialOrganizer,
		...
		// we don't want ourselves in the attendee list, since we're using it to track updates.
		...
		if (this._organizer && this._attendees.size === 0) {
	...
	private hasOtherAttendees() {
		return (
			...
			!this.isNew &&
			// if the event is not new, but the attendee list was empty, there are no attendees that are not either us or the organizer
			this.initialAttendees.size > 0
		)
	}
	...
	/**
	 * modify your own attendance to the selected value. is a no-op if we're not actually an attendee.
	 * @param status
	 */
	...
	get result(): AttendanceModelResult {
		...
		} = this.isNew ? { added: Array.from(this._attendees.values()), deleted: [], kept: [] } : trisectingDiff(this.initialAttendees, this._attendees)

		const allAttendees: Array<CalendarEventAttendee> = []
		let responseModel: SendMailModel | null = null
		if (this._organizer != null && this._organizer.address.address !== this._ownAttendee?.address.address) {
			allAttendees.push(this._organizer)
			responseModel = this.sendMailModelFactory()
			responseModel.addRecipient(RecipientField.TO, this._organizer.address)
		}
		if (this._ownAttendee != null) {
			allAttendees.push(this._ownAttendee)
		}
		...
		return {
			...
			organizer: this._organizer?.address ?? null,
			isConfidential: this.isConfidential,
			cancelModel: attendeesToCancel.length > 0 ? this.prepareSendModel(attendeesToCancel) : null,
...
export function canModifyOwnAttendance(eventType: EventType): boolean {
	// We can always modify own attendance in own event. Also can
	// modify if it's invite in our calendar and we are invited.
	return eventType === EventType.OWN || eventType === EventType.INVITE
}
 ...
 *
 */
export function canModifyGuests(sharedCalendar: boolean, eventType: EventType): boolean {
	// It is not allowed to modify guests in shared calendar or invite.
	return !(sharedCalendar || eventType === EventType.INVITE)
}
 ...
 */
export function trisectingDiff<T>(before: ReadonlyMap<unknown, T>, after: ReadonlyMap<unknown, T>): { kept: Array<T>; added: Array<T>; deleted: Array<T> } {
	const kept: Array<T> = []
	const added: Array<T> = []
